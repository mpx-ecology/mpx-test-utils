...
在本教程中，我们将会看到如何测试组件中的Store 数据，以及如何测试一个 store。
...
<template>
  <view class="text-align-center">
    <button bindtap="actionClick">Click</button>
  </view>
</template>
...
该 action 是 [Jest 伪造函数](https://jestjs.io/docs-Hans/mock-functions)。这些伪造函数让我们去断言该 action 是否被调用。
...
[Module](https://vuex.vuejs.org/guide/modules.html) 对于将我们的 store 分隔成多个可管理的块来说非常有用。它们也暴露 getter。我们可以在测试中使用它们。
